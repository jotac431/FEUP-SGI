...
export class XMLscene extends CGFscene {
    ...
    initLights() {
        ...
        var folder = this.gui.gui.addFolder('Lights');
        for (let i = 0; i < Object.keys(this.graph.lights).length; i++) {
            //this.scene.lights[i].setVisible(true)
            //this.scene.lights[i].update()
            console.log("PROPERTY: " + this.lights[i].enabled)
            folder.add(this.lights[i], 'enabled')
                .name(Object.keys(this.graph.lights)[i])
                .onChange(this.updateLights.bind(this))
        }
    }
