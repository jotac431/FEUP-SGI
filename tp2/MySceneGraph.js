...
import { MyAnimation, MyKeyframe } from './MyAnimation.js';
...
export class MySceneGraph {
    ...
    parseXMLFile(rootElement) {
        ...
        if ((index = nodeNames.indexOf("animations")) == -1){
            this.onXMLMinorError("tag <animations> is not present");
        }else {
    ...
    parsePrimitives(primitivesNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (primitiveType == 'rectangle') {
                ...
                if (parts_u == null || parts_v == null)
                    return "[Primitives] Missing values for patch. Primitive id: " + primitiveId;
                else if (isNaN(parts_u) || isNaN(parts_v))
                    return "[Primitives] Invalid values for patch. Primitive id: " + primitiveId;
                ...
                for (let i = 0; i < degree_u+1; i++) {
                    ...
                    for (let j = 0; j < degree_v+1; j++) {
                        let V = [];
                        if (patchChildren[ind].nodeName == "controlpoint") {
                            var x = this.reader.getFloat(patchChildren[ind], 'x');
                            var y = this.reader.getFloat(patchChildren[ind], 'y');
                            var z = this.reader.getFloat(patchChildren[ind], 'z');

                            if (x == null || y == null || z == null)
                                return "[Primitives] Missing values for controlpoint. Primitive id: " + primitiveId;
                            else if (isNaN(x) || isNaN(y) || isNaN(z))
                                return "[Primitives] Invalid values for controlpoint. Primitive id: " + primitiveId;
                                V.push(x, y, z, 1);
                            U.push(V);
                        }
                        else {
                            this.onXMLMinorError("[Primitives] unknown tag <" + children[i].nodeName + ">");
                            continue;
                        }
                        ind++;
                    }
                ...
                var patch = new MyPatch(this.scene, degree_u, degree_v, parts_u, parts_v, controlpoints);
    ...
    parseAnimations(animationsNode) {
        ...
        // Any number of keyframe animations.
        for (var i = 0; i < children.length; i++) {
            ...
            for (var j = 0; j < grandChildren.length; j++){
                ...
                var timestamp = this.reader.getFloat(grandChildren[j],"instant");
                grandgrandChildren = grandChildren[j].children;
                if (grandgrandChildren.length != 5){
                    return "Invalid number of transformations in keyframe " + j + " in animation " + animationId;
                }
                var transX = this.reader.getFloat(grandgrandChildren[0],'x')
                var transY = this.reader.getFloat(grandgrandChildren[0],'y')
                var transZ = this.reader.getFloat(grandgrandChildren[0],'z')
                if (this.reader.getString(grandgrandChildren[1],'axis') != 'z'){
                    return "Invalid rotation in keyframe " + j + " in animation " + animationId + ". First rotation must be around the z axis!"
                }
                var rotX = this.reader.getFloat(grandgrandChildren[1],'angle')
                if (this.reader.getString(grandgrandChildren[2],'axis') != 'y'){
                    return "Invalid rotation in keyframe " + j + " in animation " + animationId + ". Second rotation must be around the y axis!"
                }
                var rotY = this.reader.getFloat(grandgrandChildren[2],'angle')
                if (this.reader.getString(grandgrandChildren[3],'axis') != 'x'){
                    return "Invalid rotation in keyframe " + j + " in animation " + animationId + ". Third rotation must be around the x axis!"
                }
                var rotZ = this.reader.getFloat(grandgrandChildren[3],'angle')
                var scaleX = this.reader.getFloat(grandgrandChildren[4],'x')
                var scaleY = this.reader.getFloat(grandgrandChildren[4],'y')
                var scaleZ = this.reader.getFloat(grandgrandChildren[4],'z')
                keyframes.push(new MyKeyframe(vec3.fromValues(transX,transY,transZ),[rotX,rotY,rotZ],vec3.fromValues(scaleX,scaleY,scaleZ),timestamp))
            }
            this.animations[animationId] = new MyAnimation(animationId, keyframes)
   ...
   */
    parseComponents(componentsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            var highlightValues = null;
            if (highlightIndex != -1) {
                highlightValues = [];
                highlightValues[0] = this.reader.getFloat(grandChildren[highlightIndex], 'r');
                highlightValues[1] = this.reader.getFloat(grandChildren[highlightIndex], 'g');
                highlightValues[2] = this.reader.getFloat(grandChildren[highlightIndex], 'b');
                highlightValues[3] = this.reader.getFloat(grandChildren[highlightIndex], 'scale_h');
            }
            ...
            //Creating the component and adding to list of components
            this.nodes[componentID] = (new MyComponent(componentID, componentIDs, primitiveIDs, [textureID,length_s,length_t], materialIDs, transfMatrix,highlightValues,animation));
        }

        var folder = this.scene.gui.gui.addFolder('Crazy objects');
        // Add items to the folder, set name, bind on change to a function ‘method2’ in scene
        var componentNames = Object.keys(this.nodes);
        for (let i = 0; i < componentNames.length; i++) {
            //this.scene.lights[i].setVisible(true)
            //this.scene.lights[i].update()
            if (this.nodes[componentNames[i]].highlightValues != null) {
                folder.add(this.nodes[componentNames[i]], 'highlightActive')
    ...
    displayScene() {
        if (this.scene.startTime == null){
            this.scene.delta = 0;
        }else{
            this.scene.delta = Math.sin((Date.now() - this.scene.startTime) * 0.001 * Math.PI);
        }
    ...
    processComponent(comp, parent_active_mat, parent_text) {
        ...
        //Also compute the animations
        var animMatrix = comp.computeAnimation();
        if (animMatrix != null){
        ...
        var lenght_sToSet = null;
        var lenght_tToSet = null;
        var textureToSetName = null;
        //Case in which no texture defined previously and inherit required
        if (comp.texture[0] == 'inherit' && parent_text == null) {
            textureToSet = null
            textureToSetName = null
        //If set to inherit, the component should inherit the texture from parent
        } else if (comp.texture[0] == 'inherit' && parent_text != null) {
            textureToSet = this.textures[parent_text[0]];
            textureToSetName = parent_text[0];
            lenght_sToSet = parent_text[1];
            lenght_tToSet = parent_text[2];
        //If there is a new texture to set
        } else if (comp.texture[0] != 'none') {
            textureToSet = this.textures[comp.texture[0]];
            textureToSetName = comp.texture[0];
            lenght_sToSet = comp.texture[1];
            lenght_tToSet = comp.texture[2];
        }

        //Setting shaders
        if (comp.highlightValues != null && comp.highlightActive && this.scene.activeShader != this.scene.highlightShader) {
            if(lenght_sToSet != null && lenght_tToSet != null){
                this.textures[textureToSetName].bind()
                this.scene.highlightShader.setUniformsValues({ delta: this.scene.delta, r: comp.highlightValues[0], g: comp.highlightValues[1], b: comp.highlightValues[2], scale_h: comp.highlightValues[3], draw_text: true});
            }else{
                this.scene.highlightShader.setUniformsValues({ delta: this.scene.delta, r: comp.highlightValues[0], g: comp.highlightValues[1], b: comp.highlightValues[2], scale_h: comp.highlightValues[3], draw_text: false});
            }
            this.scene.setActiveShader(this.scene.highlightShader);
        } else if (this.scene.activeShader != this.scene.defaultShader && (comp.highlightValues == null || (comp.highlightValues != null && !comp.highlightActive))) {
        ...
        for (var i = 0; i < comp.primitives.length; i++) {
            if (lenght_sToSet != null && lenght_tToSet != null){
                //if (comp.primitives[i] instanceof MyRectangle || comp.primitives[i] instanceof MyTriangle)
                    this.primitives[comp.primitives[i]].updateTexCoords([lenght_sToSet, lenght_tToSet]);
            }
        ...
        for (var i = 0; i < comp.components.length; i++) {
            ...
            this.processComponent(this.nodes[comp.components[i]], materialToSetName, [textureToSetName,lenght_sToSet,lenght_tToSet]) //comp.components[i] is the name of the node, not the node itself
